...
import {
  ...
  getCompanyBranchOptionsService,
  getCompaniesService,
...
export const getCompanyBranchOptionsController = async (_req: Request, res: Response) => {
  try {
    const branches = await getCompanyBranchOptionsService();
    return res.status(200).json({ message: SUCCESS.DATA_FETCHED, data: branches });
  } catch (error: any) {
    return res.status(400).json({ error: error.message || ERROR.INTERNAL_SERVER_ERROR });
  }
};
