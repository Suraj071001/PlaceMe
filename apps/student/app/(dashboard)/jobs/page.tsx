...
export default function JobsPage() {
  ...
  const filteredJobs = useMemo(() => {
    let result = allJobs.filter((job) => {
      ...
      if (filters.search) {
        ...
        const matchesSearch =
          job.company.toLowerCase().includes(q) || job.role.toLowerCase().includes(q) || (job.skills && job.skills.some((s) => s.toLowerCase().includes(q)));
        if (!matchesSearch) return false;
      }

      // Min CGPA
      if (filters.minCgpa && job.minCgpa !== null && job.minCgpa !== undefined) {
        if (job.minCgpa < parseFloat(filters.minCgpa)) {
          return false;
        }
      }
  ...
  return (
    <div className="mx-auto max-w-6xl space-y-4 pb-10">
      ...
      <JobFilterBar filters={filters} onFilterChange={setFilters} resultCount={filteredJobs.length} locations={locations} jobTypes={jobTypes} />
      ...
      {isLoading ? (
        ...
        <div className="py-12 text-center text-muted-foreground">No jobs match your filters. Try adjusting your criteria.</div>
      ) : (
