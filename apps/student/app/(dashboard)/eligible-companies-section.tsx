...
export function EligibleCompaniesSection({
  companies,
}: EligibleCompaniesSectionProps) {
  return (
        ...
        {companies.map((company) => (
          ...
          >
            ...
            <div className="flex sm:flex-col items-center sm:items-end justify-between gap-4 self-stretch">
              <span
                className={`inline-flex items-center rounded-md px-2.5 py-0.5 text-xs font-semibold ${statusConfig[company.status]}`}
              >
                {company.status}
              </span>
              <Button className="w-auto bg-indigo-600 hover:bg-indigo-700 text-white rounded-lg px-6 shrink-0 h-9">
                Apply
              </Button>
            </div>
          </div>
